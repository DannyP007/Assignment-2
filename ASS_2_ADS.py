...
def readFile(x):
    '''
        ...
        Returns
        -------
        df_renew : variable for storing csv file
        df_renew_t : variable for storing transposed csv file

    '''
    df_renew = pd.read_csv("Renewable Energy.csv");
    df_renew = df_renew.drop(['Country Code', 'Indicator Name', \
                                'Indicator Code',], axis=1)

    return df_renew 

#call the function
df_renew  = readFile("Renewable Energy.csv")
print("\nRenewable Energy: \n", df_renew)
...
def readFile(y):
    '''
        ...
        Returns
        -------
        df_energy : variable for storing csv file
        
    '''
    df_energy = pd.read_csv("energy used.csv");
    df_energy = df_energy.drop(['Country Code', 'Indicator Name', \
                                'Indicator Code',], axis=1)
    
    return df_energy 

#call the function
df_energy = readFile("energy used.csv")
print("\nRenewable Energy \n", df_energy)
...
def readFile(z):
    ...
    df_nuclear = df_nuclear.drop(['Country Code', 'Indicator Name', \
                                'Indicator Code',], axis=1)
    
    return df_nuclear
 
#call the function
df_nuclear = readFile("Electricity Produce from nuclear source.csv")
print("\nElectricity Produce from nuclear source \n", df_nuclear)
...
#call the function
df_renew = readFile("Electricity Produce From Oil Source.csv")
print("\nElectricity Produce from oil source \n", df_renew)
...
def readFile(o):
    ...
    df_population = df_population.drop(['Country Code', 'Indicator Name', \
                                'Indicator Code',], axis=1)
    
    return df_population 

#call the function
df_population = readFile("Population.csv")
print("\nPopulation growth: \n", df_population)
